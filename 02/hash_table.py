...
class HashTable:
    ...
    def delete(self, key):
        ...
        prev = None
        while item:
            if item.key == key: # If the target key to be deleted is found
                if prev is None: # If the target is the first item in self.buckets[bucket_index]
                    self.buckets[bucket_index] = item.next # Move the head of the list to the next item
                else:
    ...
    # Rehashing buckets:
    # 1. If the buckets are 70% or more used, double the bucket size.
    # 2. If the buckets are 30% or less used, halve the bucket size.
    def rehashing(self):
        if self.item_count >= self.bucket_size * 0.7: # 1.
            double = self.bucket_size * 2
            while double not in sieve: # Find prime numbers around double.
                double += 1
            ...
            return
		# Create a new bucket list with the new size
        new_buckets = [None] * new_bucket_size
        for item in self.buckets: # Traverse the entire hash table.
            while item: # Traverse the linked list at the current bucket.
